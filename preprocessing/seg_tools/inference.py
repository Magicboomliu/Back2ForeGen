...
if __name__ == "__main__":
    ...
    # ============================================ #
    print("Loaded the Models from {}".format(opt.ckpt))
    ...
    for i, path in enumerate(tqdm(sorted(glob.glob(f"{opt.data}/*.*")))):
        ...
        mask = get_mask(model, img, use_amp=not opt.fp32, s=opt.img_size) # [H,W,1]
        mask[mask<0.8]= 0

        background_mask = np.ones_like(mask) - mask*1.0
        background_mask_saved = np.squeeze(background_mask)
        background_mask_saved[background_mask_saved>0.2]=1.0
        background_mask_saved[background_mask_saved<=0.2]=0.0

        basename = os.path.basename(path)

        saved_name = os.path.join(opt.out,basename)
        skimage.io.imsave(saved_name,(background_mask_saved*255.).astype(np.uint8))

        
        # # wget the foreground object
        # if opt.only_matted:
        #     img = np.concatenate((mask * img + 1 - mask, mask * 255), axis=2).astype(np.uint8)
        #     img = cv2.cvtColor(img, cv2.COLOR_RGBA2BGRA)
        #     saved_matted_name = os.path.join(opt.out,basename)
        #     cv2.imwrite(saved_matted_name, img)
