...
def latents_to_image(final_latents,vae,original_size,save_name=None):
    with torch.no_grad():
        ...
        inverted_latents_image_pil.save(save_name)
...
class StableDiffusionReferencePipeline(
    ...
    def __call__(
        ...
        # 8.1 define noise inversion
        @torch.no_grad()
        def ddim_noise_inversion(start_latents=None,ddim_prompt=prompt,ddim_negative_prompts=negative_prompt,
                                 ddim_prompt_embeds=None,
                                 ddim_negative_prompt_embeds=None,
                ...
                prompt_embeds = self._encode_prompt(
                            ...
                            num_images_per_prompt,
                            do_classifier_free_guidance,
                            ddim_negative_prompts,
                            prompt_embeds=ddim_prompt_embeds,
                            negative_prompt_embeds=ddim_negative_prompt_embeds,
                            lora_scale=text_encoder_lora_scale)
        ...
        # get inverted_latents
        inverted_latents = ddim_noise_inversion(start_latents=ref_image_latents)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i>len(inverted_latents):
                    ...
                    ref_xt = inverted_latents[-1].unsqueeze(0)
                else:
                    ref_xt = inverted_latents[-i].unsqueeze(0)
                
                
                if ref_xt.shape[0]<latent_model_input.shape[0]:
