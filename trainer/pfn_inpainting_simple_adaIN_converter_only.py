...
from networks.residual_adaIN_converter import AdaIN_Converter_Residual
...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        if args.adaIN_converter_type=='simple':
            current_adaIN_converter = F2All_Converter(mid_channels=1280)
        elif args.adaIN_converter_type=='residual':
            current_adaIN_converter = AdaIN_Converter_Residual(mid_channels=1280)
    ...
    current_adaIN_converter.train() # train the converter
    ...
    optimizer = optimizer_cls(
        current_adaIN_converter.parameters(),
    ...
    unet, optimizer, train_loader, test_loader,lr_scheduler,current_adaIN_converter = accelerator.prepare(
        unet, optimizer, train_loader, test_loader,lr_scheduler,current_adaIN_converter
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        current_adaIN_converter.train() # FIXME: Specific the Model we want to fixed.
        train_loss = 0.0
        for step, batch in enumerate(train_loader):
            with accelerator.accumulate(current_adaIN_converter):
                ...
                fg2all_mean_bank, fg2all_variance_bank = current_adaIN_converter(mean_bank = mean_bank_fore,
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        unwarped_model = accelerator.unwrap_model(current_adaIN_converter)
