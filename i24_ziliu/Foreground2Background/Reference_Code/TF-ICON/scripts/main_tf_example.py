...
def convert_tensor_into_image(tensor):
    ...
    tensor = (tensor[0,:,:,:].permute(1,2,0).cpu().numpy()+1.0)/2

    return (tensor*255).astype(np.uint8)

def segmentation_mask_vis(tensor):
    B,C,H,W = tensor.shape
    tensor = (tensor[0,0,:,:].cpu().numpy())*255
...
def load_img(path, SCALE, pad=False, seg=False, target_size=None):
    if seg:
        ...
        image = Image.open(path).convert("RGB") # get the image
        seg_map = Image.open(seg).convert("1") # 用于将图像转换为二值模式（即黑白模式）

        # Get the width and height of the original image
        w, h = image.size

        # Calculate the aspect ratio of the original image: get the currmy scale ratio
        aspect_ratio = h / w
...
def main():
    ...
    print("loaded all the models and DPMSolverSampler")

    #--------------------Read all the Images from Here---------------------------------#
    prompt = 'a cartoon animation of a sheep in the forest'

    opt.init_img = "/mnt/nfs-mnj-home-43/i24_ziliu/dataset/TF_ICON/dataset/example_inputs/bg58.png" # background image
    opt.ref_img = "/mnt/nfs-mnj-home-43/i24_ziliu/dataset/TF_ICON/dataset/example_inputs/fg35_63d22cda1f5b66e8e5aca776.jpg"
    opt.mask = "/mnt/nfs-mnj-home-43/i24_ziliu/dataset/TF_ICON/dataset/example_inputs/mask_bg_fg.jpg"
    opt.seg = "/mnt/nfs-mnj-home-43/i24_ziliu/dataset/TF_ICON/dataset/example_inputs/fg35_mask.png"
    ...
    scale = round(scale, 2) # this ratio is used for what?
    ...
    # 进行降采样，只保留每隔 8 个像素点的值，从而将原始分辨率降低到原来的 1/8。
    #[1,4,H,W].---> for latent, so duplicate into 4
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            for prompts in data:
                # here the prompt is just a [string]
                print(prompts)
                # c is the prompt condition input 
                # uc is the uncondition input
                # this is used for the CFG
                # there are all the shape of [1,77,1024], 77 is max_tokens numbers
                c, uc, inv_emb = load_model_and_get_prompt_embedding(model, opt, device, prompts, inv=True)
                
                T1 = time.time()
                # background image latents
                init_latent = model.get_first_stage_encoding(model.encode_first_stage(init_image)) # (1,4,64,64)
                # foreground image latents
                ...
                # background latents shape [dim, height/8, width/8]----> [4,64,64]
                shape = [init_latent.shape[1], init_latent.shape[2], init_latent.shape[3]]

                # background noise inversion to X(0)_m
                ...
                # foreground noise inversion to X(0)_m
                ...
                # save the original Z-enc background
                samples_orig = z_enc.clone()

                # inpainting in XOR region of M_seg and M_mask with pure noise
                ...
                # keep the background regions and for the transition regions, using guassain noise
                # skimage.io.imsave("SA1.png",segmentation_mask_vis(segmentation_map[:, :, top_rr:bottom_rr, left_rr:right_rr]))
                # skimage.io.imsave("SA2.png",segmentation_mask_vis((1 - segmentation_map[:, :, top_rr:bottom_rr, left_rr:right_rr])))


                # all the following shapes should be [1,4,64,64]
                samples_for_cross = samples_orig.clone() #backgroud latents: without inpainting
                samples_ref = z_ref_enc.clone()# foreground latents
                samples = z_enc.clone()# after inpaint at the transition regions with noise.
                ...
                # here the mask is the M_user, where we want to put the foreground object in
