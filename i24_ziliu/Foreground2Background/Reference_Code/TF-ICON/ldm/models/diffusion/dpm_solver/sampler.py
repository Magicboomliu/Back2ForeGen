...
class DPMSolverSampler(object):
    ...
    def sample(self,
        ...
        # just make sure the conditional prompt is the same size with the batch size
        ...
        if x_T is None:
            # if X_T is None, than initial with a guassain noise
            x = torch.randn(size, device=device)
        else:
            x = x_T

        # NoiseScheduleVP 是噪声调度器，用于处理扩散过程中的噪声进度（或时间步长）。
        # state using ns, and the alphas_cumprod
