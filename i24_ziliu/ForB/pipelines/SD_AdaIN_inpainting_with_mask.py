...
def masked_var_mean(hidden_states, mask):
    mask = mask.to("cuda:0")
    mask = mask.expand_as(hidden_states)  # [B, C, H, W]
    masked_hidden_states = hidden_states * mask  # [B, C, H, W]
    masked_sum = masked_hidden_states.sum(dim=(2, 3), keepdim=True)  # [B, C, 1, 1]
    masked_count = mask.sum(dim=(2, 3), keepdim=True)  # [B, C, 1, 1]
    mean = masked_sum / masked_count

    squared_diff = (masked_hidden_states - mean) ** 2
    masked_squared_diff_sum = squared_diff.sum(dim=(2, 3), keepdim=True)  # [B, C, 1, 1]
    var = masked_squared_diff_sum / masked_count
    return var, mean
...
class StableDiffusionReferenceInpaintingMaskPipeline(
    ...
    def _load_converter_pre_train(self,pretrained_path):
        ...
        self.converter_network = F2All_Converter(mid_channels=1280)
    ...
    def __call__(
        ...
        pretrained_model_path ="/mnt/nfs-mnj-home-43/i24_ziliu/i24_ziliu/ForB/outputs/SD_Gen_Images/sd15_anime_specific_vae_with_mask/ckpt_4001.pt"
        #pretrained_model_path = "/mnt/nfs-mnj-home-43/i24_ziliu/i24_ziliu/ForB/outputs/SD_Gen_Images/sd15_anime/ckpt_5001.pt"
        # pervious to do : load the converter
        self._load_converter_pre_train(pretrained_path=pretrained_model_path)
        ...
        # prompt embeding is [2,77,168]: 
        # previous is the uncond prompts
        # next is the true prompts
        ...
        # 7. Prepare reference latent variables: using VAE
        ...
        # dimension: first is the True, then False

        #hacked_basic_transformer_inner_forward 函数是对 BasicTransformerBlock 中 forward 函数的一种修改，
        # 用于在模型的自注意力机制中引入自定义的逻辑，以实现特定的生成效果或控制。
        # 这段代码主要用于调整 Transformer 的前向传播过程，尤其是在处理自注意力时的行为
        ...
        def hacked_mid_forward(self, *args, **kwargs):
            ...
            if MODE == "write":
                if gn_auto_machine_weight >= self.gn_weight:
                    ...
                    current_size_h = x.shape[-2]
                    current_size_w = x.shape[-1]
                    current_fg_mask = F.interpolate(fg_mask,size=(current_size_h,current_size_w),
                                                    mode='nearest')
                    var,mean = masked_var_mean(hidden_states=x,
                                                mask=current_fg_mask)

                    var = var.type_as(original_image_latents)
                    mean = mean.type_as(original_image_latents)
            ...
            for i, (resnet, attn) in enumerate(zip(self.resnets, self.attentions)):
                ...
                if MODE == "write":
                    if gn_auto_machine_weight >= self.gn_weight:
                        ...
                        current_size_h = hidden_states.shape[-2]
                        current_size_w = hidden_states.shape[-1]
                        current_fg_mask = F.interpolate(fg_mask,size=(current_size_h,current_size_w),
                                                        mode='nearest')
                        var,mean = masked_var_mean(hidden_states=hidden_states,
                                                    mask=current_fg_mask)
            ...
            for i, (resnet, attn) in enumerate(zip(self.resnets, self.attentions)):
                ...
                if MODE == "write":
                    if gn_auto_machine_weight >= self.gn_weight:
                        ...
                        current_size_h = hidden_states.shape[-2]
                        current_size_w = hidden_states.shape[-1]
                        current_fg_mask = F.interpolate(original_foreground_mask,size=(current_size_h,current_size_w),
                                                        mode='nearest')
                        var,mean = masked_var_mean(hidden_states=hidden_states,
                                                    mask=current_fg_mask)
        ...
        if reference_attn:
            ...
            for i, module in enumerate(attn_modules):
                # 对每个 BasicTransformerBlock 模块，
                # 将其原始的 forward 函数保存为 _original_inner_forward。这样做是为了在需要时能够恢复原始的 forward 行为。
                module._original_inner_forward = module.forward
                # 将 hacked_basic_transformer_inner_forward 函数绑定到当前模块，替换其原始的 forward 函数。
                # 通过这种方式，后续在模型前向传播时，将调用自定义的 hacked_basic_transformer_inner_forward 函数。
                module.forward = hacked_basic_transformer_inner_forward.__get__(module, BasicTransformerBlock)
                #为每个模块初始化一个空的 bank 列表。这是为了在“写入”模式下存储中间状态，供“读取”模式下使用。
                module.bank = []
                #为每个模块设置一个 attn_weight 属性，这个属性是根据模块在 attn_modules 中的位置计算的比例值。
                # 位置越靠前的模块，attn_weight 越小，位置越靠后的模块，attn_weight 越大。这可能用于控制自注意力机制中的权重调整。
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if use_converter:
                    ...
                    fg2all_mean_bank, fg2all_variance_bank = self.converter_network(mean_bank = mean_bank_fore,
