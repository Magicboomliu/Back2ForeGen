...
class Mid_Block_Network(nn.Module):
    def __init__(self, in_channels, time_embed_dim=512, text_prompt_dim=768):
        ...
        self.conv1 = nn.Conv2d(in_channels=self.in_channels, out_channels=self.in_channels // 2, kernel_size=3, padding=1)
        self.relu = nn.ReLU()
        ...
        # Fully Connected Layers for variance
        self.fc3 = nn.Linear(self.in_channels, self.in_channels // 2)
        self.fc4 = nn.Linear(self.in_channels // 2, self.in_channels)
        ...
        # Foreground Mask Convolutional layers
        self.mask_conv = nn.Conv2d(1, self.in_channels // 2, kernel_size=3, padding=1)
        self.mask_conv2 = nn.Conv2d(self.in_channels // 2, self.in_channels, kernel_size=3, padding=1)
    ...
    def forward(self, mid_block_mean_list, mid_block_var_list, mid_block_feat_list, time_embed, text_prompt, foreground_mask):
        ...
        x2 = mid_block_mean.view(mid_block_mean.size(0), -1)
        x2 = self.fc1(x2)
        x2 = self.relu(x2)
        x2 = self.fc2(x2)
        x2 = x2.view(x2.size(0), x2.size(1), 1, 1)

        # Process mid_block_var
        x3 = mid_block_var.view(mid_block_var.size(0), -1)
        x3 = self.fc3(x3)
        x3 = self.relu(x3)
        x3 = self.fc4(x3)
        ...
        mask = self.mask_conv(foreground_mask)
        mask = self.relu(mask)
        mask = self.mask_conv2(mask)
        mask = torch.mean(mask, dim=(2, 3), keepdim=True)  # Global average pooling

        # Combine all features
        x = x1 + x2 + x3 + time_embed + text_prompt + mask
...
class CrossAttnDownBlock2D_Network(nn.Module):
    def __init__(self, in_channels_list=[320, 640, 1280], time_embed_dim=512, text_prompt_dim=768):
        ...
        self.projection_layers_2 = nn.ModuleList()
        ...
        # Foreground Mask Convolutional layers
        self.mask_conv = nn.Conv2d(1, in_channels_list[0] // 2, kernel_size=3, padding=1)
        self.relu = nn.ReLU()
        self.mask_conv2 = nn.Conv2d(in_channels_list[0] // 2, in_channels_list[0], kernel_size=3, padding=1)
        ...
        for in_channels in self.in_channels_list:
            ...
            # Fully connected layers for x3 (variance)
            self.variance_layers.append(nn.Sequential(
                nn.Linear(in_channels, in_channels // 2),
                nn.ReLU(),
                nn.Linear(in_channels // 2, in_channels)
            ))
            ...
            self.projection_layers_2.append(
                nn.Conv2d(in_channels_list[0],in_channels,kernel_size=1, padding=0)
            )
    ...
    def forward(self, 
        ...
        # Process foreground mask
        foreground_mask_processed = self.mask_conv(foreground_mask)
        foreground_mask_processed = self.relu(foreground_mask_processed)
        foreground_mask_processed = self.mask_conv2(foreground_mask_processed)
        foreground_mask_processed = torch.mean(foreground_mask_processed, dim=(2, 3), keepdim=True)  # Global average pooling
        ...
        for i in range(len(inputs)):
            ...
            x3 = self.variance_layers[i](x3)
            x3 = x3.view(x3.size(0), x3.size(1), 1, 1)  # Reshape to [B, C, 1, 1]
            ...
            current_foreground_mask_processed = self.projection_layers_2[i](foreground_mask_processed)

            combined = x1 + x2 + x3
            current_dim = x1.shape[1]
            combined1 = combined + current_time_embed_processed + current_text_prompt_processed + current_foreground_mask_processed
            combined2 = combined1
...
class CrossAttnUpBlock2D_Network(nn.Module):
    def __init__(self, in_channels_list=[1280, 640, 320], time_embed_dim=512, text_prompt_dim=768):
        ...
        # Foreground Mask Convolutional layers
        self.mask_conv = nn.Conv2d(1, in_channels_list[-1] // 2, kernel_size=3, padding=1)
        self.relu = nn.ReLU()
        self.mask_conv2 = nn.Conv2d(in_channels_list[-1] // 2, in_channels_list[-1], kernel_size=3, padding=1)
